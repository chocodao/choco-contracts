...
contract BarsBondDepository is Ownable {
    ...
    IERC20 public immutable Bars; // token given as payment for bond
    IERC20 public immutable principle; // token used to create bond
    ITreasury public immutable treasury; // mints Bars when receives principle
    ...
    struct Bond {
        uint payout; // Bars remaining to be paid
    ...
    function deposit( 
        ...
        require( payout >= 10000000, "Bond too small" ); // must be > 0.01 Bars ( underflow protection )
        ...
        uint balanceBefore = Bars.balanceOf(address(this));
        /**
            principle is transferred in
            approved and
            deposited into the treasury, returning (_amount - profit) Bars
         */
        ...
        if ( fee != 0 ) { // fee is transferred to dao 
            Bars.safeTransfer( DAO, fee ); 
        }
        require(balanceBefore.add(profit) == Bars.balanceOf(address(this)), "Not enough Bars to cover profit");
    ...
    function stakeOrSend( address _recipient, bool _stake, uint _amount ) internal returns ( uint ) {
        if ( !_stake ) { // if user does not want to stake
            Bars.transfer( _recipient, _amount ); // send payout
        } else { // if user wants to stake
            if ( useHelper ) { // use if staking warmup is 0
                Bars.approve( address(stakingHelper), _amount );
                stakingHelper.stake( _amount, _recipient );
            } else {
                Bars.approve( address(staking), _amount );
    ...
    function maxPayout() public view returns ( uint ) {
        return Bars.totalSupply().mul( terms.maxPayout ) / 100000 ;
    }
    ...
    /**
     *  @notice calculate current ratio of debt to Bars supply
     *  @return debtRatio_ uint
     */
    function debtRatio() public view returns ( uint debtRatio_ ) {   
        uint supply = Bars.totalSupply();
    ...
    /**
     *  @notice calculate amount of Bars available for claim by depositor
     *  @param _depositor address
    ...
    /**
     *  @notice allow anyone to send lost tokens (excluding principle or Bars) to the DAO
     *  @return bool
     */
    function recoverLostToken(IERC20 _token ) external returns ( bool ) {
        require( _token != Bars, "NAT" );
