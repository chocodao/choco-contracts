...
contract BarsBondingCalculator is IBondingCalculator {
    ...
    function markdown( address _pair ) external view returns ( uint ) {
        ...
        uint reserve;
        if ( IUniswapV2Pair( _pair ).token0() == address(Bars) ) {
            reserve = reserve1;
        } else {
            require(IUniswapV2Pair( _pair ).token1() == address(Bars), "not a Bars lp pair");
            reserve = reserve0;
        }
        return reserve.mul( 2 * ( 10 ** Bars.decimals() ) ).div( getTotalValue( _pair ) );
    }
}
